...
var DateUtil = {
    // 6、字符串转日期
    str2date: function str2date(str, sep) {
        sep = [sep] || ['-', '.', '/'];
        for (var i in sep) {
    ...
    // 10、判断两个日期相差的天数
    countBetweenDate: function countBetweenDate(d1, d2) {
        if (typeof d1 === 'string') {
            d1 = str2date(d1);
        }
        if (typeof d2 === 'string') {
    ...
    // 11、获得n天以后的日期
    getDayAfter: function getDayAfter(n) {
        var now = new Date();
...
function addEvent(obj, eventname, func, isCapture) {
    if (window.VBArray) {
        ...
        addEvent = function addEvent(obj, eventname, func) {
            obj.attachEvent('on' + eventname, func);
        };
    } else {
        obj.addEventListener(eventname, func, !!isCapture);
        addEvent = function addEvent(obj, eventname, func, isCapture) {
            obj.addEventListener(eventname, func, !!isCapture);
        };
    }
}
...
var Cookie = {
    ...
    set: function set(key, value, expires, path) {
